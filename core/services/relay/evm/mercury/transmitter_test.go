...
var sampleFromAccount = utils.RandomAddress()
var sampleContractAddress = utils.RandomAddress()

var expectedMercuryReport = fmt.Sprintf(`{
  "ReportCtx": {
  ...
  "FromAccount": "%s",
  "ContractAddress": "%s"
}`, strings.ToLower(sampleFromAccount.Hex()), strings.ToLower(sampleContractAddress.Hex()))

func Test_MercuryTransmitter_Marshal(t *testing.T) {
	mr := MercuryReport{sampleReportCtx, sampleReport, sampleSigs, sampleFromAccount, sampleContractAddress}
...
func Test_MercuryTransmitter_Transmit(t *testing.T) {
	...
	t.Run("successful HTTP POST", func(t *testing.T) {
		...
		mt := NewTransmitter(lggr, sampleFromAccount, sampleContractAddress, httpClient, reportURL, username, password)
